...
export function getDashboardData(date: string): DashboardData {
  ...
  const wellnessAvg = (3.2 + (seed % 12) / 10).toFixed(1);
  const wellnessUp = seed % 3 !== 0;

  const loads = PLAYERS.map((p) => ({ p, load: getEffectiveDailyLoad(p, date, dt) }));
  const anyReal = loads.some(({ load }) => load.isReal);

  const teamAal = anyReal
  ...
  return {
    ...
    wellnessSeries: {
      ...
      fatigue: seededScale5(9, 3.4, days, seed),
    },
    alerts,
    dailyTable,
    comment: COMMENT_TEMPLATES[seed % COMMENT_TEMPLATES.length],
    usingRealData: anyReal,
  };
}
